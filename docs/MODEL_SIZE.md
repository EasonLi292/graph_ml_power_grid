...
`n_layers = 7`, `conv_type = "admittance"`). Every number below is generated by
[scripts/model_params.py](../scripts/model_params.py) — run it to reproduce.*
...
- **Depth is cheap, width is expensive.** Going 3 → 7 layers at `h=64` roughly
  *doubles* the model (435k → 1.00M); doubling width 64 → 128 nearly *quadruples*
  it (1.00M → 3.97M). Since depth is the dominant accuracy lever (see
  [MODEL_REPORT.md](MODEL_REPORT.md) §4.1), the model buys its performance with
  the cheaper axis.
- The default 1.0M-parameter / 4 MB model is tiny by modern standards — it fits
  in L2/L3 cache and runs inference in milliseconds on CPU, which is the whole
  point of a fast surrogate.
