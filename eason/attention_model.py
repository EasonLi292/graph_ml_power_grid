...
class SuperpositionAttention(nn.Module):
    ...
    With ``time_values=True`` each load also contributes its binned
    current waveform w_j[b] (stage 1, F5/F6): per head the cache gains a
    temporal slab ``T_c = Σ_j ψ_c(h_j) · (r_j ⊗ w_j) ∈ R^{m×B}``, read as
    a per-node droop *profile* ``p_i = r_iᵀ T_c ∈ R^B`` — the sketched
    quasi-static v_i(t) — then pooled (max |·|, mean) into 2 scalars per
    head. Time bins are basis-free, so basis invariance is preserved.
    """

    def __init__(self, h: int, heads: int = 4, time_values: bool = False) -> None:
        super().__init__()
        self.heads = heads
        ...
        self.out = nn.Linear(heads * h + (2 * heads if time_values else 0), h)
        self.norm = nn.LayerNorm(h)
    ...
    def forward(self, x, r, load_mask, wave=None):
        # x: [N, h]; r: [N, m]; load_mask: [N] bool; wave: [N, B] or None
        xl, rl = x[load_mask], r[load_mask]              # [J, ·]
        v = self.v(xl)                                   # [J, h]
        phi, psi = self.phi(x), self.psi(xl)             # [N, H], [J, H]
        alpha, beta = self.alpha(x), self.beta(xl)
        wl = wave[load_mask] if (self.time_values and wave is not None) else None
        outs = []
        for c in range(self.heads):
            ...
            if wl is not None:
                T = (rl * psi[:, c:c + 1]).T @ wl        # [m, B] temporal cache
                p = r @ T                                # [N, B] sketched v_i(t)
                pooled = torch.cat([p.abs().max(-1, keepdim=True).values,
                                    p.mean(-1, keepdim=True)], dim=-1)
                outs.append(phi[:, c:c + 1] * pooled)    # [N, 2]
...
class IBMAttnRegressor(nn.Module):
    ...
    def __init__(self, hidden_dim: int = 64, n_conv: int = 2, heads: int = 4,
                 in_dim: int | None = None, time_values: bool = False) -> None:
        super().__init__()
        h = hidden_dim
        # +1: log ‖r_i‖² (self-impedance, basis-invariant)
        self.proj = nn.Linear((in_dim or NODE_FEATURE_DIM) + 1, h)
        self.pre = nn.ModuleList([ConvBlock(h) for _ in range(n_conv)])
        self.attn = SuperpositionAttention(h, heads, time_values=time_values)
        self.post = nn.ModuleList([ConvBlock(h) for _ in range(n_conv)])
        self.head = nn.Sequential(nn.Linear(h, h), nn.ReLU(), nn.Linear(h, 1))
        # floor-preserving init: with the residual target, ŷ ≡ 0 IS the
        # physics baseline — start exactly there, not at random (stage-0
        # runs started below their own floor and never recovered OOD).
    ...
    def forward(self, data) -> torch.Tensor:
        ...
        wave = getattr(data["node"], "wave", None)
        for blk in self.pre:
            x = blk(x, data)
        x = self.attn(x, r, data["node"].load_mask, wave=wave)
